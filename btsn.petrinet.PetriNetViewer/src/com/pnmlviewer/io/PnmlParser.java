...
public class PnmlParser {
    ...
    private static PnmlModel parseDocument(Document doc) throws Exception {
        ...
        for (int i = 0; i < arcNodes.getLength(); i++) {
            ...
            // Read custom waypoints from graphics/position elements
            try {
                NodeList positions = (NodeList) xp.evaluate("./pnml:graphics/pnml:position", e, XPathConstants.NODESET);
                if (positions.getLength() > 0) {
                    a.waypoints = new java.util.ArrayList<>();
                    for (int j = 0; j < positions.getLength(); j++) {
                        ...
                        if (!xs.isEmpty() && !ys.isEmpty()) {
                            ...
                            a.waypoints.add(new Point2D.Double(x, y));
                        }
                    }
                }
    ...
    public static void writeWithPositions(PnmlModel model, File out) throws Exception {
        ...
        for (PnmlModel.Arc a : model.arcs) {
            ...
            // Write custom waypoints to graphics/position elements
            if (a.waypoints != null && !a.waypoints.isEmpty()) {
                Element graphics = doc.createElementNS(NS, "graphics");
                for (Point2D.Double wp : a.waypoints) {
                    Element pos = doc.createElementNS(NS, "position");
                    pos.setAttribute("x", Double.toString(wp.x));
                    pos.setAttribute("y", Double.toString(wp.y));
                    graphics.appendChild(pos);
                }
                el.appendChild(graphics);
            }
