...
class TransitionHandler {
    ...
    TransitionHandler(ServiceThread serviceThread) {
        this.serviceThread = serviceThread;
        this.logger = Logger.getLogger(TransitionHandler.class.getName());
    }
    ...
    void processXorType(String service, String operation, ArrayList<?> sargs) {
        ...
        // EDGE FALLBACK: If no conditions matched and only ONE destination exists,
        // treat as EdgeNode (simple pass-through) - supports mixed workflows where
        // different versions may not have matching decision values
        if (matchingBranchIndices.isEmpty() && nextServiceCollection.size() == 1) {
            logger.info("XOR-DIAG: No conditions matched, but single destination exists - using EdgeNode fallback");
            matchingBranchIndices.add(0);
        }
        ...
        int parentTokenId = serviceThread.getSequenceID();
        
        // Calculate the BASE parent (workflow base) for genealogy
        int baseParentId = parentTokenId - (parentTokenId % 100);
        
        // If parent token (ends in 00), start at 1 to create children
        // If child token (from join), start at 0 to preserve winner's ID
        int branchNumber = (parentTokenId % 100 == 0) ? 1 : 0;
        ...
        if (isMultiMatch && !matchingBranchIndices.isEmpty()) {
            ...
            serviceThread.instrumentTokenExit(parentTokenId, serviceThread.getServiceName(),
                                              firstService, firstOp, "ForkNode", cleanRoutingPath);
        }

        for (int branchIndex : matchingBranchIndices) {
            // Create child token ID for fork, OR preserve actual token ID for single-match
            // FIX: For single-match, use phaseSequenceID (actual incoming token) not parentTokenId
            // This ensures retry loops correctly track the same token through iterations
            int newSequenceId = isMultiMatch ? (parentTokenId + branchNumber) : serviceThread.getPhaseSequenceID();
            ...
            if (isMultiMatch) {
                serviceThread.instrumentForkChild(baseParentId, newSequenceId, serviceThread.getServiceName());
                serviceThread.instrumentTokenExit(newSequenceId, serviceThread.getServiceName(),
                                                  nextService, nextOp, "ForkNode", cleanRoutingPath);
            } else {
                // Single-match XOR: record T_out EXIT event
                // FIX: Use getPhaseSequenceID() for instrumentation - the actual incoming token ID
                // newSequenceId equals parentTokenId which may be wrong after JOIN reconstitution
    ...
    void processGatewayType(String service, String operation, ArrayList<?> sargs) {
        ...
        // treat as EdgeNode (simple pass-through) - supports mixed workflows where
        // different versions may not have matching decision values
        if (matchingIndices.isEmpty() && nextServiceCollection.size() == 1) {
            logger.info("GATEWAY-DIAG: No conditions matched, but single destination exists - using EdgeNode fallback");
    ...
    private void executeGatewayForkByIndices(List<Integer> matchingIndices, String attributeValue, String arcValue) {
        ...
        int parentTokenId = serviceThread.getSequenceID();
        logger.info("GATEWAY-FORK: Parallel split to " + joinCount + " destinations, parent token " + parentTokenId + " [arc=" + arcValue + "]");
        ...
        for (int branchIndex : matchingIndices) {
            // Create child token ID for fork (inline - no TokenIdManager dependency)
            int childTokenId = parentTokenId + branchNumber;
            ...
            // PETRI NET: Record fork child genealogy and exit (use BASE parent)
            serviceThread.instrumentForkChild(baseParentId, childTokenId, serviceThread.getServiceName());
        ...
        logger.info("GATEWAY-FORK: Created " + joinCount + " child tokens from parent " + parentTokenId);
    }
    ...
    private void executeGatewayEdgeByIndex(int matchingIndex, String attributeValue, String arcValue) {
        ...
        if ("TERMINATE".equals(nextService) && "TERMINATE".equals(nextOp)) {
            ...
            serviceThread.instrumentTokenTerminate(serviceThread.getPhaseSequenceID(), serviceThread.getServiceName(), "GatewayNode");
            return;
        }
     ...
     * 
     * FIX: Use queryCanonicalReturnAttribute() to get the SOURCE's return attribute.
     * 
     * The canonical binding structure is: canonicalBinding(operation, returnAttr, inputAttr)
     * 
     * For FORK routing:
     *   - Source sends its RETURN attribute
     *   - Destination expects this as its INPUT attribute
     *   
     * Example:
     ...
     *   -> P2 expects "token" as input
     *   
     *   P1 forks to P2: sends "token" -> P2 receives "token" ✓
     * 
     * PREVIOUS BUG: Used queryCanonicalBindingSlots() which returned INPUT slots
     * (token_branch1, token_branch2) instead of the RETURN attribute (token).
     */
    void processForkType(String service, String operation, ArrayList<?> sargs) {
        ...
        int sequenceID = serviceThread.getSequenceID();
        
        // Calculate the BASE parent (workflow base) for genealogy
        // e.g., 1000001 -> 1000000, 1000000 -> 1000000
        int baseParentId = sequenceID - (sequenceID % 100);

        logger.info("FORK: Parallel split to " + joinCount + " service(s)");
        
        // Query the RETURN attribute from this service's canonical binding.
        // This is what we send to downstream services (matches their INPUT attribute).
        // FAIL LOUDLY if not found - no silent defaults.
        String forkAttribute;
        try {
            forkAttribute = serviceThread.queryCanonicalReturnAttribute(operation);
            logger.info("FORK: Using return attribute '" + forkAttribute + "' from canonical binding");
        } catch (IllegalStateException e) {
            logger.error("FORK: FATAL - " + e.getMessage());
            logger.error("FORK: Token " + sequenceID + " DROPPED - routing aborted");
            return;
        }

        // If parent token (ends in 00), start at 1 to create children
        // If child token (from join), start at 0 to preserve winner's ID
        int branchNumber = (sequenceID % 100 == 0) ? 1 : 0;
        
        for (ServiceThread.ServiceRoute route : allServices) {
            // Create child token ID for fork (inline - no TokenIdManager dependency)
            int childTokenId = sequenceID + branchNumber;
            serviceThread.getHeaderMap().put("sequenceId", Integer.toString(childTokenId));

            logger.info("FORK: Routing child token " + childTokenId + " to " + route.serviceName +
                       "." + route.operationName + " with attribute '" + forkAttribute + "'" +
                       " (branch=" + branchNumber + ")");

            // PETRI NET: Record fork child genealogy (use BASE parent, not current token)
            serviceThread.instrumentForkChild(baseParentId, childTokenId, serviceThread.getServiceName());
            
            // PETRI NET: Record EXIT event for this child token showing its specific destination
            // This allows the animator to trace each forked token to its target place
            serviceThread.instrumentTokenExit(childTokenId, serviceThread.getServiceName(),
                                              route.serviceName, route.operationName,
                                              "ForkNode", forkAttribute);

            // Route with the return attribute from canonical binding
            serviceThread.routeToServiceNoRecord(route, val, branchNumber, forkAttribute);
            branchNumber++;
        }
