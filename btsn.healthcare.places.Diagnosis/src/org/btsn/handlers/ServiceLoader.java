 ...
 * 
 * UPDATED: Added -service filter to load only specific services
 * 
 * Usage:
 *   java ServiceLoader -version v001                     # Load ALL services
 *   java ServiceLoader -version v001 -service P1_Place   # Load ONLY P1_Place
 *   java ServiceLoader -version v999 -service P1_InitializationService  # Load ONLY init service
 */
public class ServiceLoader {
    private OOjdrewAPI oojdrew;
    private static String buildVersion = null;  // Must be explicitly provided via command line
    private static String serviceFilter = null; // NEW: If set, only load services matching this name
    ...
    private static void startShutdownFileWatcher() {
        try {
            ...
            // Include service filter in filename if specified
            String filePrefix = serviceFilter != null ? serviceFilter : "all_services";
            shutdownControlFile = new File(filePrefix + "_" + buildVersion + ".running");
     ...
     * 
     * Supported arguments:
     *   -version <version>   : Required. The rule version (e.g., v001, v999)
     *   -service <name>      : Optional. Load only this specific service
     */
    private static void parseArguments(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            } else if ("-service".equals(args[i]) && i + 1 < args.length) {
        ...
        if (buildVersion == null) {
            ...
            System.err.println("Usage: java ServiceLoader -version <version> [-service <serviceName>]");
            System.err.println("  -version <version>  : Required. Rule version (e.g., v001, v999)");
            System.err.println("  -service <name>     : Optional. Load only this service");
            System.err.println("");
            System.err.println("Examples:");
            System.err.println("  java ServiceLoader -version v001                    # Load ALL services");
            System.err.println("  java ServiceLoader -version v001 -service P1_Place  # Load only P1_Place");
            System.err.println("  java ServiceLoader -version v999 -service P1_InitializationService");
            System.exit(1);
        }
        
        // Log configuration
        logger.info("=== ServiceLoader Configuration ===");
        logger.info("  Version: " + buildVersion);
        if (serviceFilter != null) {
            logger.info("  Service Filter: " + serviceFilter + " (loading ONLY this service)");
        } else {
            logger.info("  Service Filter: NONE (loading ALL services)");
        }
    }
    
    /**
     * Check if a service should be loaded based on the filter
     * 
     * @param serviceName The service name from the LoaderQuery file
     * @return true if the service should be loaded, false if it should be skipped
     */
    private static boolean shouldLoadService(String serviceName) {
        if (serviceFilter == null) {
        ...
        // Exact match
        if (serviceName.equals(serviceFilter)) {
            return true;
        }
        
        // Partial match (e.g., filter "Init" matches "P1_InitializationService")
        if (serviceName.contains(serviceFilter)) {
            return true;
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (serviceFilter != null) {
            logger.info("*** SELECTIVE LOADING: Only loading service: " + serviceFilter + " ***");
        }
        ...
        for (File ruleMLFile : ruleMLFiles) {
            // Extract service name from filename FIRST
            String queryServiceName = extractServiceNameFromFile(ruleMLFile.getName());
            
            // NEW: Check if this service should be loaded based on filter
            if (!shouldLoadService(queryServiceName)) {
                logger.info("SKIPPING service: " + queryServiceName + " (filter: " + serviceFilter + ")");
                servicesSkipped++;
                continue;
            }
            
            logger.info("LOADING service: " + queryServiceName);
        ...
        logger.info("Services loaded: " + servicesLoaded);
        logger.info("Services skipped: " + servicesSkipped);
        if (serviceFilter != null) {
            logger.info("Filter applied: " + serviceFilter);
        }
        ...
        logger.info("  1. Delete file: " + (shutdownControlFile != null ? shutdownControlFile.getName() : "N/A"));
        logger.info("  2. Send 'SHUTDOWN' to UDP port: " + shutdownPort);
        logger.info("  3. Kill the process");
        
        // Keep main thread alive while services run
        while (!shutdownRequested.get()) {
            try {
                Thread.sleep(5000);
            } catch (InterruptedException e) {
        ...
        logger.info("=== ServiceLoader Main Thread Exiting ===");
    }

    /**
     * Helper class to hold service operation data
     */
    static class ServiceOperation {
        String serviceName;
        String operation;
        String channelId;
        String port;
        ...
        boolean isComplete() {
            return serviceName != null && operation != null && 
                   channelId != null && port != null;
        }
    }

    /**
     * Create ServiceThread for a single operation
     */
    private static void createServiceThreadForOperation(ServiceOperation op) {
        ...
        try {
            ...
            logger.info("Setting up " + op.serviceName + "." + op.operation + 
                " -> EventReactor:" + eventReactorPort + ", RuleHandler:" + ruleHandlerPort);
